; ModuleID = '/home/jbonfigs/EECE5698_FinalProject/hestonEuro_hls/solution1/.autopilot/db/a.g.ld.5.gdce.bc'
source_filename = "llvm-link"
target datalayout = "e-m:e-i64:64-i128:128-i256:256-i512:512-i1024:1024-i2048:2048-i4096:4096-n8:16:32:64-S128-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024"
target triple = "fpga64-xilinx-none"

; Function Attrs: inaccessiblemem_or_argmemonly noinline willreturn
define void @apatb_hestonEuro_ir(float* noalias nocapture nonnull "maxi" %pCall, float* noalias nocapture nonnull "maxi" %pPut, float %expect, float %kappa, float %variance, float %correlation, float %timeT, float %freeRate, float %volatility, float %initPrice, float %strikePrice) local_unnamed_addr #0 {
entry:
  %pCall_copy = alloca float, align 512
  %pPut_copy = alloca float, align 512
  call fastcc void @copy_in(float* nonnull %pCall, float* nonnull align 512 %pCall_copy, float* nonnull %pPut, float* nonnull align 512 %pPut_copy)
  call void @apatb_hestonEuro_hw(float* %pCall_copy, float* %pPut_copy, float %expect, float %kappa, float %variance, float %correlation, float %timeT, float %freeRate, float %volatility, float %initPrice, float %strikePrice)
  call void @copy_back(float* %pCall, float* %pCall_copy, float* %pPut, float* %pPut_copy)
  ret void
}
...
declare void @apatb_hestonEuro_hw(float*, float*, float, float, float, float, float, float, float, float, float)
...
define void @hestonEuro_hw_stub_wrapper(float*, float*, float, float, float, float, float, float, float, float, float) #4 {
entry:
  call void @copy_out(float* null, float* %0, float* null, float* %1)
  call void @hestonEuro_hw_stub(float* %0, float* %1, float %2, float %3, float %4, float %5, float %6, float %7, float %8, float %9, float %10)
  call void @copy_in(float* null, float* %0, float* null, float* %1)
  ret void
}

declare void @hestonEuro_hw_stub(float*, float*, float, float, float, float, float, float, float, float, float)
